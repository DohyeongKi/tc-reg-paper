...
the shell scripts used to run them on our server. You may run all 
experiments at once using the master shell script 
`run_all_analyses.sh`, or run individual experiments using their 
corresponding shell scripts. The role of each R script is described 
below. Running these R scripts requires the R package `regmdc`. 
Installation instructions can be found on the package's GitHub
