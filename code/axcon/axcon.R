...
#' Fit an axially concave (or convex) regression model to data
#' 
#' This function fits an axially concave (or convex) regression model to data 
#' by solving the finite-dimensional least squares problem (104) in 
#' Section 10.2 of the supplementary material.
#'
#' @param X_design A numeric design matrix. Each row corresponds to one 
#'   observation.
#' @param y A numeric vector of responses.
#' @param is_concave A logical scalar indicating whether to fit the axially
#'   concave model (`TRUE`) or the axially convex model (`FALSE`).
#' @param is_scaled A logical scalar indicating whether the design matrix is
#'   already scaled so that every entry lies between 0 and 1. If `FALSE`,
#'   min–max scaling is applied to each column of the design matrix.
...
#' @details 
#' If both `concave_covariates` and `convex_covariates` are `NULL`, they are 
#' determined by `is_concave`. For example, if `is_concave` is `TRUE`, 
#' `concave_covariates` is the whole set of covariates while `convex_covariates` 
#' is `NULL`. If exactly one of `concave_covariates` and `convex_covariates` is 
#' not `NULL`, the one that is `NULL` is converted to the complement of the 
#' other. If both of them are not `NULL`, their union must cover all covariates.
axcon <- function(X_design, y,
                  is_concave = TRUE, is_scaled = FALSE, rounding_digit = 4L,
  ...
  # ============================================================================
  # Apply min-max scaling if necessary. After scaling, round each entry of the 
  # design matrix.
  if (is_scaled) {
  ...
  # Construct the position indicator matrix M in equation (104) of Section 10.2 
  # of the supplementary material. Also, compute the sorted unique entries for 
  # each column of the design matrix (0 and 1 are additionally included)
  ...
  # Construct the constraint matrices for each coordinate that encode the 
  # finite difference constraints in equation (102) of Section 10.2 of the 
  # supplementary material
  constraint_matrices <- get_constraint_matrices(unique_entries)

  # Solve the finite-dimensional optimization problem (104) of Section 10.2 of 
  ...
  # Compute the fitted values at design points
  fitted_values_design <- position_indicator_matrix %*% fitted_values_lattice
  
  # Return the fitted axially concave (or convex) regression model
  axcon_model <- list(
    X_design = X_design,
    y = y,
