#' Produce predictions at new data points
#'
#' Given a fitted model produced by \code{\link{axcon}}, this function computes 
#' predictions at new data points by performing the multi-affine interpolation 
#' described in equation (103) of Section 10.2 of the supplementary material 
#' 
#' @param axcon_model A fitted axially concave/convex regression model returned 
#'   by \code{\link{axcon}}.
#' @param X_pred A numeric matrix of new data points. Each row corresponds to 
#'   one observation at which a prediction will be made. 
predict_axcon <- function(axcon_model, X_pred) {
  ...
  predictions <- sapply((1L:nrow(X_pred)), function(row) {
    ...
    # Perform the multi-affine interpolation described in (103) of Section 10.2 
